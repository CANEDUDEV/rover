/*
 * Mayor implementation. The idea is that the user will define some parameters
 * for their mayor as specified by the ck_mayor_t struct, then the library will
 * initialize its state from these parameters when the user call ck_mayor_init.
 * Then, ck_process_kings_letter can be used to act on commands from the king.
 * The postmaster implementation is responsible for parsing CAN messages and
...
/*
 * Struct contains pointers to parameters that should be defined by the
 * user. The parameters are used by the mayor library to e.g. set the various
 * modes.
 */
typedef struct {
  ...
  // would just overflow.
  uint64_t ean_no;     // 40-bit EAN-13 number.
  uint64_t serial_no;  // 40-bit serial number.
  ...
  // Can be set by the mayor if known beforehand.
  // Otherwise the king will set it.
  uint32_t base_no;
  ...
  // CAN Kingdom for the king's document and the mayor's document, respectively.
  ck_folder_t *folders;
  ...
  // and line form lists. The mandatory lists are the transmit document list and
  // the receive document list, which both need to have capacity 1. Record no 0
  // in the transmit document list and the receive document list is reserved by
 ...
 * 1.
 * The function defines the mayor's document, then assigns the king's document
 * and the mayor's document to folders 0 and 1, respectively. It also
 */
